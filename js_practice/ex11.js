 ...
 */
users.every((user) => user.email.match(/.+@[a-zA-Z0-9]+\.[a-zA-Z]+/) ? true : false)
 ...
 */
console.log(users.filter((user) => parseInt(/([0-9]+)\.[0-9]+\.[0-9]+\.[0-9]+/.exec(user.ip_address)[1]) < 128 ? true : false).length)
...
users.findIndex((user) => {
    const secondOctet = parseInt(/([0-9]+)\.[0-9]+\.[0-9]+\.[0-9]+/.exec(user.ip_address)[1])
    if(secondOctet > 127 && secondOctet < 192) {
        return true;
    }
    return false;
})
 ...
 */
users.filter((user) => user.email.match(/.+@soundcloud\.[a-zA-Z]+/) ? true : false)
