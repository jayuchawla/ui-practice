...
users.find((user) => user.id === comments.find((Comment) => Comment.text === 'OK great thanks').userId)
...
comments.map((comment) => {
    ...
    // comments = [...comments.find((comment) => comment.userId === iterableUser.id), firstName = iterableUser.firstName]
    return { ...comment, firstName, lastName }
})

// 5) Get a list of the users who haven't commented ***IMP***
users.filter((user) => !comments.some((comment) => comment.userId === user.id))